...
class ItemModule:
    ...
    def delete_by_user(name):
        conn = sqlite3.connect(ItemModule.DB)
        query = "DELETE FROM {table} WHERE name=?".format(table=ItemModule.TABLE_NAME)
    ...
    def update(self):
        conn = sqlite3.connect(self.DB)
        query = "UPDATE {table} SET {col2}=? WHERE {col1}=?".format(table=self.TABLE_NAME,col1=self.COL_1,col2=self.COL_2)
